let my_wishlist     = '',
    ...
    sl              = 'aefc56e6-fdf5-4379-8eff-fbff72bb83d2',
...
$(function(){
	...
	if(window.location.href.indexOf('?lid=2f1edbb5-6bad-4c0d-8b54-6f083fc6b30d')!=-1){
...
function init_wishlist(){
	...
	$('body').on('click',class_item, function(e){
		...
		if(that.hasClass('active')){
			...
			console.log('Add');
			update_wishlist(pid,'add');
			//Confirm message add to wishlist
			$("#addWishlist").fadeIn(1000);
		    $("#addWishlist").fadeOut(1500);
		}
	});
...
function load_add_to_cart_wishlist(){
	...
	$('body').on('change', '.wishlist__shelf-item-qty-input', function(){
		var rest_qty=$(this).parent().find('.wishlist__shelf-item-qty-rest');
		var qty=parseInt($(this).val());
		if(qty<=0 || isNaN(qty)){
			$(this).val(1);
			
			rest_qty.addClass('show-disabled');
		}
	});
	...
	$('body').on('click', '.wishlist__shelf-item-qty-plus', function(e){
		...
		if(!isNaN(qty)){
			qty++;
			console.log(qty, max);
			rest_qty.removeClass('show-disabled');
			if(qty>max)
				qty=max
			input_qty.val(qty);
		}else{
