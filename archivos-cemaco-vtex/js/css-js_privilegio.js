	...
	if(data.IsUserDefined){
		...
		$.get('/api/dataentities/CL/search?_fields=tarjetaPrivilegio&_where=(email='+profile_email+')',function(privtex){
			if(privtex.length && privtex[0].tarjetaPrivilegio!=null && privtex[0].tarjetaPrivilegio!=''){
				$('#privilegio-actions').html('<p class="font-primary">Tu tarjeta privilegio es: '+privtex[0].tarjetaPrivilegio+'</p>');
				$('#priv__create, #priv__link').remove();
			}
			$('#privilegio-actions').addClass('active');
		});
	}else{
		$('#privilegio-actions').addClass('active');
	}
	...
	$.ajax({
		...
		success: function(data){
			...
			if(data.Result.Res){
				...
				$('#priv__state-form-balance').html(''+
					'<div class="text-center" style="margin:auto;">'+
						'<h4 class="tag-title--level-3 font-white">'+data.Data.Nombre+'</h4>'+
						'<h3 class="tag-title--level-2 font-white">Saldo: Q'+parseFloat(data.Data.Saldo).formatMoney(2, decimal, separator)+'</h3>'+
					'</div>');
			}else{
				$('#priv__state-form-balance .control-wrapper--send').before('<div class="error">Hubo un error: '+data.Result.Mensaje+'</div>');
			}
		}
	});
}
function load_get_balance(){
	jq2('#balanceBirthdate').prop('readonly', true).datepicker({
		dateFormat: "dd/mm/yy",
		altField: "#balanceBirthdate-alt",
		altFormat: "yymmdd",
		changeMonth: true,
		changeYear: true,
		yearRange: '1944:2000',
	...
	$('#priv__state-form-balance').on('submit', function(e){
		e.preventDefault();
		var tp=$('#balanceCard').val();
		var birthdate=$('#balanceBirthdate-alt').val();
...
$(function(){
	$.get('/no-cache/profileSystem/getProfile',function(data){
		...
		if(data.IsUserDefined){
			profile_email=data.Email;
			load_get_balance();
			$.get('/api/dataentities/CL/search?_fields=email&_where=(email='+profile_email+')',function(md_data){
				if(md_data.length==0){
					window.location.href='/_secure/account?privilegio=1';
				}
			});
		}else{
			$('#priv__create, #priv__link').empty();
			vtexid.setScope('2384422b-df02-41e4-b38f-037b5a69b331');
			vtexid.setScopeName(jsnomeSite);
			var priv_login_selectors='.expandircrea, .scrollCrea, .expandeEnlaza, .scrollEnlaza, #balanceCard, #balanceBirthdate, #priv__state-form-balance button';
			$('body').off('click', priv_login_selectors).on('click', priv_login_selectors, function (e) {
				e.preventDefault();
				window.location.href='/_secure/account?privilegio=1';
				// vtexid.start({
				// 	locale: 'es-MX',
...
// ----------------- Show/Hide Privilegio Forms ----------------- //
//privi
//Boton crear desplegar formulario
$( ".expandircrea" ).click(function() {
  $( ".formprivi_enlaza" ).hide();
  $( ".formprivi_crea" ).slideDown();
  $( ".expandircrea" ).hide();
  $( ".scrollCrea" ).show();
  $( ".scrollEnlaza" ).hide();
  $( ".expandeEnlaza" ).show();
});
$( ".scrollCrea" ).click(function() {
  $([document.documentElement, document.body]).animate({
    scrollTop: $(".titularp").offset().top
  }, 1000);
});
$( ".regresa_crear-privi" ).click(function() {
  $( ".formprivi_crea" ).slideUp();
  $( ".scrollCrea" ).hide();
  $( ".expandircrea" ).show();
});
//Boton enlaza desplegar formulario
$( ".expandeEnlaza" ).click(function() {
  $( ".formprivi_crea" ).hide();
  $( ".formprivi_enlaza" ).slideDown();
  $( ".expandeEnlaza" ).hide();
  $( ".scrollEnlaza" ).show();
  $( ".scrollCrea" ).hide();
  $( ".expandircrea" ).show();
});
$( ".scrollEnlaza" ).click(function() {
  $([document.documentElement, document.body]).animate({
    scrollTop: $(".titularp").offset().top
  }, 1000);
});
$( ".regresa_enlaza-privi" ).click(function() {
  $( ".formprivi_enlaza" ).slideUp();
  $( ".scrollEnlaza" ).hide();
  $( ".expandeEnlaza" ).show();
});
// ----------------- END Show/Hide Privilegio Forms ----------------- //
