...
jQuery.fn.vtexSmartResearch=function(opts)
{
    ...
    var defaults=
	{
		...
		emptySearchMsg:'<h3>Intenta filtrar nuevamente, la combinaci\u00F3n de filtros no tiene resultados.</h3>', // Html com a mensagem para ser apresentada quando nÃƒÂ£o existirem resultados para os filtros selecionados
    ...
    var options=jQuery.extend(defaults, opts),
		...
		currentSearchUrl="",
		itemsPerPage= 24,
		lidId="c444d71b-1ae9-47f0-8b74-d24a14223dbb"
		urlFilters="",
	...
	var fn=
	{
		getUrl:function(scroll)
		{
			var s=scroll||false;
			if(s)
				return currentSearchUrl.replace(/#[0-9]*/g,"").replace(/PageNumber=[0-9]*/,"PageNumber="+currentPage);
			else{
				var number = parseInt(window.location.hash?window.location.hash.replace("#",""):"1")
				if(number)
					return (searchUrl+urlFilters).replace(/#[0-9]*/g,"").replace(/PageNumber=[0-9]*/,"PageNumber="+number);
				return (searchUrl+urlFilters).replace(/#[0-9]*/g,"").replace(/PageNumber=[0-9]*/,"PageNumber="+pageNumber);
			}
		},
		getSearchUrl:function()
		{
			...
			if(typeof(url)!=="undefined" && typeof(url[0])!=="undefined"){
				number = parseInt(window.location.hash?window.location.hash.replace("#",""):"1")
				if(number)	
					return url[0].replace(/PageNumber=[0-9]*/,"PageNumber="+number)
				return url[0]
			}
			else
			{
		...
		configPaginator:function(pageCount)
		{
			if(!pageCount){
				var tmp		
				elementPages=body.find(".pager:first").attr("id");
				tmp=(elementPages||"").split("_").pop();
				pageCount=(null!==options.pageLimit)?options.pageLimit:window["pagecount_"+tmp];
			}
			var elementsPagination = body.find(".pagination")
			var limit = 4
			if(pageCount<limit)
				limit = pageCount
			var pageNumber = parseInt(window.location.hash?window.location.hash.replace("#",""):"1")

			pageNumber = pageNumber>pageCount?1:pageNumber
			if(elementsPagination){
				elementsPagination.each(function(i,val){
					val.setAttribute("active",pageNumber)
					val.setAttribute("pages",pageCount)
				})
			}
			if(pageCount){
				body.find(".pagination").each(function(i,pag){
					var visible = parseInt(pag.getAttribute("visibleValues")??0)
					var initialValue = pageNumber - visible
					if(initialValue<1)
						initialValue=1
					var pagination = jQuery(pag)
					var pages = pagination.children(".pagination-item")
					pages.each((index,val)=>{
						if(pages.length > pageCount && index>=pageCount){
							jQuery(val).remove()
						}
						else{
							if((initialValue+index) === pageNumber){
								val.setAttribute("class",val.getAttribute("class").replace(/ *active/g,"")+" active")
							}
							else {
								val.setAttribute("class",val.getAttribute("class").replace(/ *active/g,""))
							}
							jQuery(val).text(index+initialValue)
						}
					})
					if(pageCount===1)
						pagination.find(".pagination-nav").css("display","none")
					else
						pagination.find(".pagination-nav").css("display","flex")
				})
			}

		},
		getPage: function(page){
			var currentContent=loadContentE.find(options.shelfClass).filter(":last");
			currentContent.after(elemLoading);
			currentStatus=false;
			if($('.fix-anchor-pagination').length){
				var targetScroll = $('.fix-anchor-pagination').offset().top;
				_html.animate({scrollTop: targetScroll + 'px'},"slow");
			}else{
				_html.animate({scrollTop:0},"slow");
			}
			pageJqxhr=jQuery.ajax({
				url: fn.getUrl(true).replace(/PageNumber=[0-9]*/,"PageNumber="+page),
				success:function(data)
				{
					window.history.pushState({},"",window.location.href.replace(/#[0-9]*/g,"").concat(`#${page}`))
					if(data.trim().length<1)
					{
						moreResults=false;
						log("No hay mÃ¡s resultados a partir de la pÃ¡gina: "+(page),"Aviso");
					}
					else{
		...
		pagination:function(callback,pages=null)
		{
            $(window).resize(function(){
                body.find(".pagination").each(function(index,val){
                    var visible =  val.getAttribute("visibleValues")
                    var indexActive = 0;
                    if($(window).width()<1200)
                    {
                        var pages =  val.getAttribute("pages")
                        var pagination = jQuery(val)
                        if(pagination.children(".pagination-item").length>4){
                            val.setAttribute("visibleValues",4)
                            visible=4
                            pagination.children(".pagination-item").each(function(i,page){
                                if(page.getAttribute("class").includes("active"))
                                    indexActive=i;
                            })
                            var indices = []
                            var pagesToRemove = []
                            ...
                            pagination.children(".pagination-item").each(function(index,val){
                                if(indices.includes(index)){
                                    pagesToRemove.push(val)
                                }
                            })
                            pagesToRemove.forEach((e)=>{e.remove()})
                        }
                    }
                    else
                    {
                        var pages =  val.getAttribute("pages")
                        var pagination = jQuery(val)
                        if(pagination.children(".pagination-item").length<6 && pages>4){
                            val.setAttribute("visibleValues",6)
                            var visible=6
                            if(pages<visible)
                                visible=pages
                            var active = pagination.find(".pagination-item.active").text()
                            var startValue = parseInt(pagination.children(".pagination-item").first().text())
							var actualCount = pagination.children(".pagination-item").length
                            var endValue = startValue+5;
                            if(endValue>pages){
                                endValue=pages;
                                startValue=pages-5
                            }
                            if(startValue<1)
                                startValue=1
                            var addItem = visible - actualCount
							if(addItem+actualCount>endValue){
								addItem = endValue - actualCount
							}
                            var nextButton = pagination.children(".pagination-nav").last()
                            nextButton.detach()
                            
                            for(var i=0;i<addItem;i++){
                                var item = $("<button>"+i+"</button>")
                                item.attr("class","pagination-item")
                                item.click({element:val, button:item,callback},simple_pagination_change)
                                item.appendTo(pagination)
                            }
                            pagination.children(".pagination-item").each(function(index,val){
                                var newValue = index + startValue
                                if(newValue!==parseInt(active)){
                                    val.setAttribute("class",val.getAttribute("class").replace(/ *active/g,""))
                                }
                                else{
                                    val.setAttribute("class",val.getAttribute("class").replace(/ *active/g,"")+" active")
			...
			var elementsPagination = body.find(".pagination")
			if(elementsPagination){
				elementsPagination.each(function(index,val){
                    var visible = 6
                    if($(window).width()<1200)
                        visible=4
                    val.setAttribute("visibleValues",visible)
                    var max = parseInt(val.getAttribute("pages")), active = parseInt(val.getAttribute("active")??0)
					active=parseInt(window.location.hash?window.location.hash.replace("#",""):"1")
                    var idIcon = val.getAttribute("icon")
                    if(idIcon){
                        if(!jQuery("#"+idIcon).html()){
                            idIcon=undefined
                        }
                    }
                    var limit = max, init = 1 
                    if(!active)
                        active=1
                    if(visible){
                        if(max<visible){
					...
					var pagination = jQuery(val)
					if(pagination.find(".pagination-nav.prev").length<1){
						var button = $("<button class=\"pagination-nav prev\"></button>")
						button.click({element:val, factor:-1,callback},simple_pagination_navigate)
						button.appendTo(val)
						if(idIcon){
							$("<svg class=\"svg-icon icon\"><use href=\"#"+idIcon+"\"></use></svg>").appendTo(button)
						}
					}
					var actualPagesItemCount = pagination.children(".pagination-item").length
					for(i=init;i<=limit;i++){
						if(i>=actualPagesItemCount+init){//CHECK
							var item = $("<button>"+i+"</button>")
							if(i===active)
								item.attr("class","pagination-item active")
							else
								item.attr("class","pagination-item")
							item.click({element:val, button:item,callback},simple_pagination_change)
							item.appendTo(val)
					...
					if(pagination.find(".pagination-nav:not(.prev)").length<1){
						button = $("<button class=\"pagination-nav\"></button>")
						button.click({element:val, factor:1,callback},simple_pagination_navigate)
						if(idIcon){
							$("<svg class=\"svg-icon icon\"><use href=\"#"+idIcon+"\"></use></svg>").appendTo(button)
						}
						...
						var prevButton = jQuery(pagination.find(".pagination-nav:not(.prev)"))
						prevButton.detach()
						prevButton.appendTo(val)
					}
					if(max<=1){
		...
		changeTotalItemsLabel: function(value){
			var label = jQuery(body.find(".product-list__results-value:first"))
			if(parseInt(value)>1){		
				label.text(`${value} Articulos`)
			}
			else if(parseInt(value)===1){		
				label.text(`${value} Articulo`)
			}
			else{
				label.text(`Sin Articulos`)
			}
		},
	...
	if($this.length<1)
	{
		...
		fn.scrollToTop();
		fn.configPaginator();
		fn.pagination(function(page){fn.getPage(page);})
		return $this;
	}
	...
	var fns=
	{
		...
		addFilterIndicator:function(filter_value){
			...
			window.history.replaceState(null, null, (current_uri+filter_indicator).replace(/#[0-9]*/,""));
		},
		...
		addFilter:function(input)
		{
			...
			prodOverlay.fadeTo(300,0.6);
			currentSearchUrl=fn.getUrl().replace(/PageNumber=[0-9]/,"PageNumber=1");
			shelfJqxhr=jQuery.ajax({
				url:currentSearchUrl,
				success:fns.filterAjaxSuccess,
				error:fns.filterAjaxError
		...
		filterAjaxSuccess:function(data)
		{
			...
			$.ajax({
				//Consulta a un template sin paginación para tener el número de articulos filtrados
				url: `${currentSearchUrl.replace('buscapagina','busca')}&lid=${lidId}`,
				success: function(data){
					var template = jQuery(data)
					//El template de busqueda tiene un tag que obtiene el total de resultados
					var totalItems =  parseInt(template.find(".resultado-busca-numero .value:first").html())
					var totalPages = null
					//Busqueda de elemento con ID de variable de número de pagina (page_count)
					var elementPages=body.find(".pager:first").attr("id");
					var tmp=(elementPages||"").split("_").pop();
					if(!Number.isNaN(totalItems))
					{
						//Calculo del total de paginas dependiendo de total de articulos, entre productos por página
						totalPages = Math.ceil(totalItems/itemsPerPage)
					}
					else{
						totalPages = 0
					}
					//Cambio de etiqueta de numero de articulos
					fn.changeTotalItemsLabel(totalItems)
					//Actualización de cantidad de paginas
					window["pagecount_"+tmp] = totalPages
					//Reconfiguración de paginador
					fn.configPaginator(totalPages)
					//Renderizado de paginador y redistribución
					fn.pagination(function(page){fn.getPage(page)})
				},
			})
...
;(function(c){
    $(window).on('load', function(){
        var $_searchMultipleNavigator   = $('.search-multiple-navigator'),
            ...
            initialStateFilters         = 'is-collapsed',
            initialStateLinks           = 'is-collapsed',
        ...
        $(  '.search-multiple-navigator h3,'+
            ...
            if( $_accordionTrigger.hasClass('is-collapsed') ){
				if( $_accordionTrigger.closest('fieldset.is-collapsed')[0]){
	            	console.log( $_accordionTrigger.closest('fieldset.is-collapsed'))
...
function simple_pagination_navigate(event){
    var active = parseInt(event.data.element.querySelector(".active").textContent)
	active=parseInt(window.location.hash?window.location.hash.replace("#",""):"1")
    var pages = parseInt(event.data.element.getAttribute("pages"))
    if(active+event.data.factor>=1 && active+event.data.factor<=pages)
    {
        if(event.data.callback){
            event.data.callback(active+event.data.factor)
        }
        handle_pagination_value(active+event.data.factor,event.data.element)
    }
}
function simple_pagination_change(event){
    var value = event.data.button.text()
    if(event.data.callback){
        event.data.callback(value)
    }
    handle_pagination_value(parseInt(value),event.data.element)
}

function handle_pagination_value(value,element){
    var pages = parseInt(element.getAttribute("pages")),
        visibleValues = element.getAttribute("visibleValues"),
        startValue = parseInt(element.querySelector(".pagination-item").textContent),
        midValue = visibleValues?Math.ceil(parseInt(visibleValues)/2):undefined
    if(visibleValues){
        if(value>pages)
           value = pages
        if(value<1)
            1 
        if(startValue+parseInt(visibleValues)<=pages){
            if(startValue+parseInt(visibleValues)-1<pages){
                startValue = (value - midValue)+1
                if(startValue<1)
                    startValue=1
                if(startValue+parseInt(visibleValues)>=pages && value>1)
        ...
        if(startValue+parseInt(visibleValues)-1===pages){
            if(value===startValue)
            {
                startValue = value - midValue + 2
            }
        }
        
        if(startValue+parseInt(visibleValues)-1>pages && parseInt(value)!==1)
            startValue=pages-parseInt(visibleValues)+1
        if(value<startValue){
            startValue=value-1
        }
        if(startValue<1)
    ...
    if(element)       
    { 
        element.querySelectorAll(".pagination-item").forEach(function(child,index){
            pageNumber = parseInt(child.textContent)
            if(child.ELEMENT_NODE===child.nodeType){
                if(midValue){
                    pageNumber = startValue+index
                    child.textContent=startValue+index
                }
                if(pageNumber===value){
                    child.setAttribute("class",child.getAttribute("class").replace(/ *active/g,"")+" active")
                }
                else if(child.getAttribute("class")){
                    child.setAttribute("class",child.getAttribute("class").replace(/ *active/g,""))
                }
            }
        })
    }
    element.setAttribute("active",value)
	if(pages === value){
		element.querySelector(".pagination-nav:last-child").style.display = 'none';
	}else{
		element.querySelector(".pagination-nav:last-child").style.display = 'flex';
	}
}
// END PAGINATOR SETTINGS
//Call functions
$(window).on('load', function(){
