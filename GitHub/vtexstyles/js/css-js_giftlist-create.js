...
$(function(){
	$('link[href*="css-js_giftlist-create.css"]').attr('href', '/files/css-js_giftlist-create.css?v='+makeid(10));
});
...
$(function(){
	...
	$('#form-create-priv button').on('click', function(e){
		e.preventDefault();
		var that=$('#form-create-priv');
		var id=that.attr('id');
		$('.error').remove();
		if(validate_ajax_priv(id)){
			dataToPost.Nombres=$('#name').val();
			dataToPost.Apellidos=$('#lastname').val();
			...
			dataToPost.NombreFactura=$('#nameCard').val();
			dataToPost.DireccionResidencia=$('#address').val();
			...
			$.ajax({
				...
				success: function(data){
					...
					if(typeof data.Result !== 'undefined' && data.Result.Res){
						...
						$('#mdr-tp').val(data.Data.Tarjeta).slideDown(500);
						extra1.fes1.tp=data.Data.Tarjeta;
						save_extra_info(admin_pos,member_pos);
						$('.mdr__privilegio-option[data-priv-have="si"]').addClass('active');
						$('.mdr__privilegio-option[data-priv-have="no"]').remove();
						setTimeout(function(){
							$('#priv__create').slideDown(500, function(){
								$(this).remove();
							});
						}, 5000);
					}else{
...
function validate_ajax_priv(id){
	console.log(id);
	$.validity.start();
...
$(function(){
	...
	if($('#birthdate').length && $('#birthdate-alt').length){
...
if($('body').hasClass('boda')){
	btn_class='boda-primary';
	is_boda=1;
	current_type='boda';
}else if($('body').hasClass('bebe')){
	btn_class='bebe-secondary';
	is_bebe=1;
	current_type='bebe';
	load_baby_functions();
	...
	is_generico=1;
	current_type='generico';
	load_generic_functions();
}
$('.button-flat.btn_bg').addClass(btn_class).removeClass('btn_bg');
/*
fes1 - Creador de la lista, admin
tp - Tarjeta privilegio
tr - Tienda de Recorrido
tl - Tienda de Liquidación
te - Tipo de entrega, valores: credito - productos
*/
var extra1={
	"fes1": {
		"dpi": "",
...
var img_functions_interval;
$(document).ready(function () {
	SaveFormClick2();
	CancelFormClick2();
	load_content_mdr();
	img_functions_interval=setInterval(function(){
		if(typeof CangeFotoInit !== 'undefined'){
			clearInterval(img_functions_interval);
			console.log('asdfsdf');
			CangeFotoInit=false;
			ValidateFoto=false;
			$(':file').off('change').on('change', function () {
				return ValidateFoto2(this.files[0]);
			});
			function ValidateFoto2(file) {
				ClearErrorMessage();
				if (file == undefined) {
					ShowErrorMessage2("Archivo no encontrado");
					$('#giftlistupload').attr('disabled', 'disabled');
					return false;
				}
				else if (file.name.length < 1) {
				}else if (file.size > 1000000) {
					ShowErrorMessage2("La imagen es demasiado grande");
					$('#giftlistupload').attr('disabled', 'disabled');
					return false;
				}else if (file.type != 'image/png' && file.type != 'image/jpg' && !file.type != 'image/gif' && file.type != 'image/jpeg') {
					ShowErrorMessage2("El formato del archivo debe ser png, jpg o gif");
					$('#giftlistupload').attr('disabled', 'disabled');
					return false;
				}
				$('#giftlistupload').attr('disabled', false);
				return true;
			}
		}
	},200);
	if($('body').hasClass('boda')){
		if($('#inputtext1member1').val()=='' && $('#inputtext1member2').val()==''){
			save_extra_info(admin_pos,member_pos);
		}else{
			load_extra_info();
		}
	}
	if($('body').hasClass('bebe')){
		if($('#inputtext1member1').val()=='' && $('#inputtext1member2').val()==''){
			save_extra_info(admin_pos,member_pos);
		}else{
			load_extra_info();
	...
	// Load input for generic list only create form
	if(is_generico && $('body').hasClass('create')){
		move_admin_member(1,2,1);
	}
	// Load input for generic list only create form
	if(is_kids && $('body').hasClass('create')){
		move_admin_member(1,2,1);
	}
	// Load tarjeta privilegio if is linked to VTEX account
	if($('body').hasClass('create')){
		load_privilegio_card();
	}
	// Generate URL
	$('body').on('keyup change', '#giftlistname', function(){
		var url=slug($('#giftlistname').val());
		$('#giftlisturl').val(url);
		$('#giftlisturl').trigger('keyup keydown');
	});
	// Change Text Validate URL duplicated
	$(document).ajaxComplete(function(event, xhr, settings){
		// if(settings.url.indexOf('/no-cache/giftlistv2/save')!=-1){
		// }
		if($('body').hasClass('edit')){
			if(settings.url.indexOf('/no-cache/giftlistv2/address/get/')!=-1){
				// Load original info for trigger
				setTimeout(function(){
					$('#original-address').val($('.address-item.active input[type="radio"]').val());
				},500);
			}
		}
	});
	// Send contract by email
	$('body').on('click', '#send-contract-email', function(e){
		e.preventDefault();
	});
	$('body').on('click', '#send-contract-email:not(.sent)', function(e){
		e.preventDefault();
		var email=$('#membermail1').val(),
			name=$('#membername1').val();
		var datos = {};
		datos.name=name;
		datos.email=email;
		datos.type=current_type;
		$('#send-contract-email').addClass('sent');
		$.ajax({
			...
			error:function(x,s){
				console.log(x);
				console.log(s);
				$('#send-contract-email').removeClass('sent');
			},
			success: function(data){
				console.log(data);
				var  msj='';
				if(current_type=='boda'){
					msj='bodas';
				}else if(current_type=='bebe'){
					msj='club bebé'
				}else if(current_type=='kids'){
					msj='evento de niños'
				}else{
					msj='festejo'
				}
				alert('El contrato de '+msj+' ha sido enviado a tu email con éxito!');
			}
		});
	});
	// Check terms
	$('#terms-check').on('change', function(){
		if($(this).is(':checked')){
			...
			$('#giftlistaccept').prop("checked", false);
		}
	});
	// Move inputs to member
	$('.mdr__member-option').on('click', function(){
		var that=$(this),
			member=that.attr('data-member');
		$('.mdr__member-option').removeClass('active');
		that.addClass('active');
		move_admin_member(1,2,member);
		extra1.admin=member;
		save_extra_info(admin_pos,member_pos);
		if(!$('#mdr__form--hide').hasClass('shown')){
			$('#mdr__form--hide').addClass('shown').slideDown(500);
		}
	});
	// Delivery type
	$('.mdr__form-delivery').on('click', function(){
		var that=$(this)
			delivery=that.attr('data-delivery-type');
		$('.mdr__form-delivery').removeClass('active');
		that.addClass('active');
		extra1.te=delivery;
		save_extra_info(admin_pos,member_pos);
	});
	// Privilegio
	$('.mdr__privilegio-option').on('click', function(e){
		e.preventDefault();
		var that=$(this)
			have_card=that.attr('data-priv-have');
		$('.mdr__privilegio-option').removeClass('active');
		if(have_card=='si'){
			$('#priv__create').stop(true, true).slideUp(500);
			$('#mdr-tp').stop(true, true).slideDown(500);
		}else{
			$('#priv__create').stop(true, true).slideDown(500);
			$('#mdr-tp').stop(true, true).slideUp(500);
		}
		that.addClass('active');
	});
	$('.mdr__privilegio-data').on('click', function(){
		var that=$(this)
			member=that.attr('data-priv-member');
		$('.mdr__privilegio-data').removeClass('active');
		that.addClass('active');
		$('#privilegio-step-2').slideDown(500);
		load_departamentos_privilegio();
	});
	$('#privilegio-depto').on('change', function(){
		var depto=$(this).val();
		load_municipios_privilegio(depto);
	});
	$('#mdr-tp').on('change keyup', function(){
		extra1.fes1.tp=$(this).val();
		save_extra_info(admin_pos,member_pos);
	});

	// Extra info 1
	$('#mdr-fes'+admin_pos+'-dpi').on('change keyup', function(){
		extra1.fes1.dpi=$(this).val();
		save_extra_info(admin_pos,member_pos);
	});
	$('#mdr-fes'+admin_pos+'-cel').on('change keyup', function(){
		extra1.fes1.cel=$(this).val();
		save_extra_info(admin_pos,member_pos);
	});
	// Extra info 2
	$('#mdr-fes'+member_pos+'-dpi').on('change keyup', function(){
		extra2.fes2.dpi=$(this).val();
		save_extra_info(admin_pos,member_pos);
	});
	$('#mdr-fes'+member_pos+'-cel').on('change keyup', function(){
		extra2.fes2.cel=$(this).val();
		save_extra_info(admin_pos,member_pos);
	});
	$('#mdr-fes2-email').on('change keyup', function(){
		extra2.fes2.email=$(this).val();
		save_extra_info(admin_pos,member_pos);
	});

	// Save Tienda para hacer tu recorrido
	$('#mdr-tr').on('change', function(){
		extra1.tr=$(this).val();
		save_extra_info(admin_pos,member_pos);
	});
	// Save Tienda donde realizarás tu liquidación
	$('#mdr-tl').on('change', function(){
		extra1.tl=$(this).val();
		save_extra_info(admin_pos,member_pos);
	});

	// loading on upload image
	$(document).ajaxSend(function(evt, request, settings) {
		if(settings.url.indexOf('/no-cache/giftlistv2/uploadimage/')!=-1) {
			$('#mdr__form-image--wrapper').addClass('sending');
		}
	});
	$(document).ajaxComplete(function(event, xhr, settings){
		if(settings.url.indexOf('/no-cache/giftlistv2/uploadimage/')!=-1) {
			$('#mdr__form-image--wrapper').removeClass('sending');
		}
	});

	// Function check
	// $('.mdr__form-check').on('click', function(e){
	// 	var that=$(this);
	// 	if(that.hasClass('checked')){
	// 		that.removeClass('checked');
	// 		$('#mdr-baby').removeAttr('disabled');
	// 	}else{
	// 		that.addClass('checked');
	// 		$('#mdr-baby').attr('disabled', 'disabled');
	// 	}
	// });
});
...
$(function(){
	...
	jq2('#giftlisteventdate').prop('readonly', true).datepicker({
		dateFormat: "dd/mm/yy",
		minDate: 1,
		maxDate: 730
	});
...
function load_departamentos_privilegio(){
	$('#privilegio-depto').empty();
	var data='<option value="">Seleccionar Departamento</option>';
	for (var k in countryData){
		mdr_states.push(k);
		data+='<option value="'+k+'">'+k+'</option>';
	}
	$('#privilegio-depto').append(data);
}
function load_municipios_privilegio(depto){
	$('#privilegio-city').empty();
	var data='<option value="">Seleccionar Municipio</option>';
	if(depto!=''){
		for (var k in countryData[''+depto]){
			data+='<option data-postal-code="'+countryData[''+depto][k]+'" value="'+k+'">'+k+'</option>';
		}
	}
	$('#privilegio-city').append(data);
}

function save_extra_info(adminpos, memberpos){
	var extra1txt=JSON.stringify(extra1),
		extra2txt=JSON.stringify(extra2);
	console.log(extra1txt.length, extra2txt.length);
	if($('body').hasClass('boda')){
		$('#inputtext1member'+adminpos).val(extra1txt);
		$('#inputtext1member'+memberpos).val(extra2txt);
	}else{
		$('#inputtext1member'+adminpos).val(extra1txt);
		$('#inputtext1member'+memberpos).val(extra2txt);
	}
...
function load_extra_info(){
	var admin=$('#gl-members .adminwrapperyes input:checked').closest('ul.giftlistul'),
		adminpos=$('#gl-members .adminwrapperyes input:checked').attr('id').replace('memberisadmin-yes','');
	var extra1txt=admin.find('.extra-info input').val();
	if(isJson(extra1txt)){
		extra1=JSON.parse(extra1txt);
	}else{
		extra1=JSON.parse(extra1txt.replace(/\s/g, '"'));
	}
	if($('#membermailadmin2').length){
		var member=$('#gl-members .adminwrapperyes input:not(:checked)').closest('ul.giftlistul'),
			memberpos=$('#gl-members .adminwrapperyes input:not(:checked)').attr('id').replace('memberisadmin-yes','');
		var extra2txt=member.find('.extra-info input').val();
		if(isJson(extra2txt)){
			extra2=JSON.parse(extra2txt);
		}else{
			extra2=JSON.parse(extra2txt.replace(/\s/g, '"'));
		}
		member_pos=memberpos;
		// Load member2 info
		$('#mdr-fes'+memberpos+'-dpi').val(extra2.fes2.dpi);
		$('#mdr-fes'+memberpos+'-cel').val(extra2.fes2.cel);
		$('#mdr-fes2-email').val(extra2.fes2.email);
	}else{
		$('.mdr__form-check-member2').remove();
		$('#mdr-fes2-name-wrapper').closest('.control-wrapper').remove();
	}
	admin_pos=adminpos;
	var pos=extra1.admin;
	console.log('adminpos',adminpos);
	console.log('memberpos',memberpos);
	console.log('pos', pos);

	// Load admin extra info
	$('#mdr-fes'+adminpos+'-dpi').val(extra1.fes1.dpi);
	$('#mdr-fes'+adminpos+'-cel').val(extra1.fes1.cel);
	// Load tipo de entrega
	$('.mdr__form-delivery').removeClass('active');
	$('.mdr__form-delivery[data-delivery-type="'+extra1.te+'"]').addClass('active');
	$('.mdr__form-delivery:not(.active)').closest('.control-wrapper').remove();
	// Load Tarjeta Privilegio
	$('#mdr-tp').val(extra1.fes1.tp);
	// Load Tiendas
	$('#mdr-tr').val(extra1.tr);
	$('#mdr-tl').val(extra1.tl);

	// Load original info for trigger
	$('#original-tr').val(extra1.tr);
	$('#original-tl').val(extra1.tl);
	$('#original-tp').val(extra1.fes1.tp);

	move_admin_member(adminpos, memberpos, pos);
	load_baby_name(adminpos);
	if(is_generico || is_kids){
		var event_type=$('#inputtext2member'+adminpos).val();
		$('#mdr-type').val(event_type);
	}
}
function isJson(str) {
	try {
...
function move_admin_member(adminpos, memberpos, pos){
	var member2=2;
	if(pos==2){
		member2=1;
	}
	console.log('Mover admin a: ', pos);
	console.log('Mover member a: ', member2);
	console.log('VTEX admin pos: ', adminpos);
	console.log('VTEX member pos: ', memberpos);
	// Move inputs Member 1 (Admin)
	// Name
	$('#membername'+adminpos).attr('placeholder', 'Nombre').appendTo('#mdr-fes'+pos+'-name-wrapper');
	// Lastname
	$('#membersurname'+adminpos).attr('placeholder', 'Apellido(s)').appendTo('#mdr-fes'+pos+'-lastname-wrapper');
	// Email
	$('#membermail'+adminpos).attr('placeholder', 'Email').appendTo('#mdr-fes'+pos+'-email-wrapper');
	// DPI
	$('#mdr-fes'+adminpos+'-dpi').appendTo('#mdr-fes'+pos+'-dpi-wrapper');
	// Celular
	$('#mdr-fes'+adminpos+'-cel').appendTo('#mdr-fes'+pos+'-cel-wrapper');
	if($('#membermailadmin2').length){
		// Move inputs Member 2
		// Name
		$('#membername'+memberpos).attr('placeholder', 'Nombre').appendTo('#mdr-fes'+member2+'-name-wrapper');
		// Lastname
		$('#membersurname'+memberpos).attr('placeholder', 'Apellido(s)').appendTo('#mdr-fes'+member2+'-lastname-wrapper');
		// DPI
		$('#mdr-fes'+memberpos+'-dpi').appendTo('#mdr-fes'+member2+'-dpi-wrapper');
		// Celular
		$('#mdr-fes'+memberpos+'-cel').appendTo('#mdr-fes'+member2+'-cel-wrapper');
		// Email
		$('#mdr-fes2-email').appendTo('#mdr-fes'+member2+'-email-wrapper');
	}
}
...
function load_content_mdr(){
	$('#mdr__form--wrapper').prependTo('#giftlistform');
	$('#errormessage').prependTo('.mdr__action--wrapper');

	// Block editing email on admin
	var container=$('#gl-members .adminwrapperyes input:checked').closest('ul.giftlistul'),
		admin_email=container.find('input[id*="membermail"]');
	admin_email.attr('readonly', 'readonly');

    // Nombre de lista
    var nameList = $('#mdr-name').attr('placeholder')
	$('#giftlistname').attr('placeholder', nameList).insertAfter('#mdr-name');
	$('#mdr-name').hide();
	// Fecha de lista
	$('#giftlisteventdate').attr('placeholder', 'DD/MM/YYYY').removeAttr('title').insertAfter('#mdr-date');
	$('#mdr-date').hide();
	// URL de lista
	if($('body').hasClass('create')){
		$('#giftlisturl').attr('placeholder', 'Escoge tu link').insertAfter('#mdr-url')
	}else if($('body').hasClass('edit')){
		$('#giftlisturl-readonly').attr('placeholder', 'Escoge tu link').insertAfter('#mdr-url')
	}
	$('#mdr-url').hide();
	// Mensaje de lista
	$('#giftlistmessage').attr('placeholder', 'Escribe un mensaje para tus invitados').insertAfter('#mdr-comments');
	$('#messagecharcounter').insertAfter('#giftlistmessage');
	$('#mdr-comments').hide();
	// Imagen de lista
	$('#giftlistimage').appendTo('#mdr__form-image');
	$('#giftlistupload').addClass('button-flat '+btn_class).val('Subir foto').insertAfter('#mdr__form-image-btn');
	$('#mdr__form-image-btn').hide();
	$('#giftlistremoveimage').val('Eliminar foto').attr('title', 'Eliminar foto').addClass('button-lineal tertiary');
	$('#giftlistimageview').insertAfter('#giftlistupload');

	// Email in message
	$('#mdr__account-email').html($('#membermail1').val());
}

function CancelFormClick2() {
    $('#btn-giftlist-cancel').off("click").click(function (e) {
    	e.preventDefault();
    	$('body').addClass('show-confirmation');
    });
    $('.confirmation-yes').on('click', function(e){
		e.preventDefault();
		if (($('#giftlistid').val() == '' || $('#giftlistid').val() == '0') && $('#giftlistfileid').val() != '') {
            RemovePhoto(function () { window.location.href = $('#giftlistredirectpath').val(); });
        } else {
            window.location.href = $('#giftlistredirectpath').val();
        }
	});
	$('.confirmation-no').on('click', function(e){
		e.preventDefault();
		$('body').removeClass('show-confirmation');
	});
}

function SaveFormClick2(){
	// $('#giftlistv2save').before('<button id="btn-giftlist-save" class="btn btn-primary">Guardar</button>').remove();
	$('#btn-giftlist-save').off("click").click(function(e){
		if($('#btn-giftlist-save').attr('clicked')){
			return;
		}
		$.validity.setup({ outputMode:"label" });

		$('#btn-giftlist-save').attr('clicked', true);
		...
		if(validate_ajax(id)){
			// $('#btn-giftlist-save').stop(true, true).fadeOut('fast');
			...
			$.ajax({
				...
				data: dataToPost,
				beforeSend: function(){
					$('#giftlistform').addClass('sending');
				},
				success: function(data){
					console.log(data);
					if(data == ''){
						if(false){
							// Data for email edit list
							var name=$('#membername1').val(),
								email=$('#membermail1').val(),
								type=$('#giftlisttype').val();
							if(type==4){
								type=type4
							}
							if(type==5){
							...
							var datos = {};
								datos.glid=$('#giftlistid').val();
								datos.name=name;
								datos.type=type;
							send_data_to_email(datos, 'edit');
						}else{
							if($('#giftlistid').length){
								var glid=$('#giftlistid').val();
								update_mdr_by_id(glid, function(){
						...
						if($('#glisitens').length > 0){
							...
							// Data for email create new list
							var name=$('#membername1').val(),
								email=$('#membermail1').val(),
								type=$('#giftlisttype').val(),
								gc; // Gift Card
							...
							var datos = {};
								...
								datos.name=name;
								datos.type=type;
							var item_to_add={"GiftListId":data,"CheckedItems":[gc],"AddToQuantity":false};
							console.log('New list created', datos);
							update_mdr_by_id(data);
							add_first_item_to_list(item_to_add, datos, 'new');
						}
					}else{
						var new_text='<div class="error">Ocurrió un error al intentar guardar la mesa de regalos, favor de verificar los datos e internar nuevamente.</div>'
						if(data.indexOf('a url escolhida já pertence a uma outra lista. url sugerida')!=-1){
							new_text=data.replace('a url escolhida já pertence a uma outra lista. url sugerida:', 'La URL ya pertenece a otra lista, URL sugerida:').replace('-','');
						}
						if(data.indexOf('a mensagem não é obrigatória mas só pode conter até 1000 caracteres')!=-1){
							new_text='<div class="error">El mensaje para personalizar tu lista de regalos no debe tener más de 1000 caracteres</div>';
						}
						ShowErrorMessage2(new_text);
						$('#btn-giftlist-save').stop(true, true).fadeIn('slow');
						$('#giftlistform').removeClass('sending');
						$('#btn-giftlist-save').removeAttr('clicked');
					}
				},
				error: function(xmlHttpRequest, textStatus){
					ShowErrorMessage2(textStatus);
					// $('#btn-giftlist-save').stop(true, true).fadeIn('slow');
					// cache
					// tex.Practices.Caching - Você utilizou um cache local mas não me deu condições de identificar a conta no contexto, não achei um httpContext. Use a chamada do método onde a conta é passada como parâmetro
					// Error status
					// 429
					// Other errors
					/*
			...
			$('html, body').animate({
				scrollTop: $('#giftlistform .error').first().offset().top - $('.site-header').height() - 150
			}, 500, function(){
				$('#giftlistform .error').fadeOut(100).fadeIn(100).fadeOut(100).fadeIn(100);
			});
			$('#btn-giftlist-save').removeAttr('clicked');
		}
		e.preventDefault();
...
function ShowErrorMessage2(message) {
    ...
    $('#errormessage').empty();
	// $('html, body').animate({ scrollTop: $('#giftlistformwrapper').offset().top }, 1500);
    $('#errormessage').html(message);
    $('#errormessage').stop(true, true).slideDown(100, function(){
    	$('#errormessage').fadeOut(100).fadeIn(100).fadeOut(100).fadeIn(100);
    });
    timeoutError = setTimeout(function () {
        $('#errormessage').stop(true, true).slideUp('slow');
    }, 30000);

    BlinkInputErrorStartStop()
    ClearInputError();
}
function send_data_to_email(datos,newedit){
	console.log('send_data_to_email', datos, newedit);
	$.ajax({
		url:'/api/dataentities/MR/documents',
		accept: 'application/vnd.vtex.ds.v10+json',
		contentType: 'application/json; charset=utf-8',
		type:'PUT',
		data: JSON.stringify(datos),
		...
		success: function(data){
			...
			var urlRedirect = $('#giftlistredirectpath').val();
			if(newedit=='new'){
				window.location.href = urlRedirect + '?new=' + datos.glid;
			}else{
				window.location.href = urlRedirect + '?update=' + $('#giftlistid').val();
			}
		}
...
function validate_privilegio(tp){
	if(tp.length==13){
		var tmp=tp.split('');
		var digit_validation=(10-(((parseInt(tmp[0])+parseInt(tmp[2])+parseInt(tmp[4])+parseInt(tmp[6])+parseInt(tmp[8])+parseInt(tmp[10]))+(parseInt(tmp[1])+parseInt(tmp[3])+parseInt(tmp[5])+parseInt(tmp[7])+parseInt(tmp[9])+parseInt(tmp[11]))*3)%10))%10;
		console.log(digit_validation);
		if(digit_validation==parseInt(tmp[12])){
...
function validate_ajax(id){
	console.log(id);
	$.validity.start();
	$('#'+id+' #mdr-type').require('Campo requerido');
	$('#'+id+' #giftlisteventdate').require('Campo requerido');
	$('#'+id+' #giftlistname').require('Campo requerido');
	$('#'+id+' #membername1').require('Campo requerido');
	$('#'+id+' #membersurname1').require('Campo requerido');
	$('#'+id+' #mdr-fes1-dpi').require('Campo requerido').minLength(13,'Deben ser 13 dígitos').maxLength(13,'Deben ser 13 dígitos');
	$('#'+id+' #mdr-fes1-cel').require('Campo requerido').minLength(8,'Deben ser 8 dígitos').maxLength(8,'Deben ser 8 dígitos');
	var memberid=$('input[id*="memberisadmin-yes"]:checked').attr('id').replace('memberisadmin-yes', '');
	$('#'+id+' #membermail'+memberid).require('Campo requerido').match('email','Debe tener formato de email, ej. nombre@email.com');
	$('#'+id+' #mdr-tp').require('La tarjeta privilegio es requerida');
	$('#'+id+' #mdr-tr').require('Campo requerido');
	$('#'+id+' #mdr-tl').require('Campo requerido');
	$('#'+id+' #giftlisturl').require('Campo requerido');
	if(($('body').hasClass('generico') && $('#check-member2.checked').length) || ($('body').hasClass('kids') && $('#check-member2.checked').length) || (!$('body').hasClass('generico') && !$('body').hasClass('kids'))){
		$('#'+id+' #membername2').require('Campo requerido');
		$('#'+id+' #membersurname2').require('Campo requerido');
		$('#'+id+' #mdr-fes2-dpi').require('Campo requerido').minLength(13,'Deben ser 13 dígitos').maxLength(13,'Deben ser 13 dígitos');
		$('#'+id+' #mdr-fes2-cel').require('Campo requerido').minLength(8,'Deben ser 8 dígitos').maxLength(8,'Deben ser 8 dígitos');
		$('#'+id+' #mdr-fes2-email').require('Campo requerido').match('email','Debe tener formato de email, ej. nombre@email.com');
	}
	$('#'+id+' #terms-check').assert( validate_privacy(), 'Debes aceptar los términos y condiciones');
	// $('#'+id+' #lastname').require('Campo requerido');
    // $('#'+id+' #validate-client').assert( validate_client(), 'Debes seleccionar una opción');

    // Validate Privilegio
    if($('#mdr-tp').val().length){
        $('#mdr-tp').assert(validate_privilegio($('#mdr-tp').val()), 'Tarjeta inválida');
    }

	// Validate Address
    $('#'+id+' #mdr-address').assert( validate_address(), 'Debes agregar una dirección de entrega');
	var result = $.validity.end();
	return result.valid;
}
function validate_privacy(){
...
function load_baby_functions(){
	$('#check-baby-name').on('click', function(e){
		e.preventDefault();
		var that=$(this);
		console.log(that.hasClass('checked'));
		if(that.hasClass('checked')){
			$('#check-baby-name').removeClass('checked');
			$('#mdr-baby').removeAttr('disabled').focus();
			console.log('Ya tengo el nombre');
		}else{
			$('#check-baby-name').addClass('checked');
			$('#mdr-baby').val('').attr('disabled', 'disabled');
			console.log('No tengo el nombre');
		}
	});
	$('#mdr-baby').on('change keyup', function(){
		var baby_name=$(this).val();
		$('#inputtext2member'+admin_pos).val(baby_name);
	});
}
function load_baby_name(adminpos){
	if(is_bebe){
		var baby_name=$('#inputtext2member'+adminpos).val();
		if(baby_name!=''){
			$('#mdr-baby').val(baby_name);
		}else{
			$('#mdr-baby').closest('.internal-form').remove();
		}
	}
}

function load_privilegio_card(){
	$.get('/no-cache/profileSystem/getProfile',function(data){
		if(data.IsUserDefined){
			$.get('/api/dataentities/CL/search?_fields=tarjetaPrivilegio&_where=(email='+data.Email+')',function(privtex){
				if(privtex.length && privtex[0].tarjetaPrivilegio!=null && privtex[0].tarjetaPrivilegio!=''){
					$('#mdr-tp').val(privtex[0].tarjetaPrivilegio);
					extra1.fes1.tp=privtex[0].tarjetaPrivilegio;
					save_extra_info(admin_pos,member_pos);
				}
			});
		}
...
function load_generic_functions(){
	$('#mdr-type').on('change', function(){
		var type=$(this).val();
		$('#inputtext2member'+admin_pos).val(type);
	});
	if(is_generico && $('body').hasClass('create')){
		$('#membername2, #membersurname2, #mdr-fes2-dpi, #mdr-fes2-cel, #mdr-fes2-email').attr('disabled', 'disabled')
	}
	$('#check-member2').on('click', function(e){
		e.preventDefault();
		var that=$(this);
		console.log(that.hasClass('checked'));
		if(that.hasClass('checked')){
			$('#check-member2').removeClass('checked');
			$('#membername2, #membersurname2, #mdr-fes2-dpi, #mdr-fes2-cel, #mdr-fes2-email').attr('disabled', 'disabled');
			$('#membernumber2,#memberid2,#memberclientid2,#memberuserid2,#membername2,#membersurname2,#membertitle2,#membermail2,#inputtext1member2,#inputtext2member2,#memberisadmin-no2,#memberisactive2').removeAttr('name');
		}else{
			$('#check-member2').addClass('checked');
			$('#membername2, #membersurname2, #mdr-fes2-dpi, #mdr-fes2-cel, #mdr-fes2-email').removeAttr('disabled');
			$('#membername2').focus();
			// Add input name
			$('#membernumber2').attr('name', 'membernumber2');
			$('#memberid2').attr('name', 'memberid2');
			$('#memberclientid2').attr('name', 'memberclientid2');
			$('#memberuserid2').attr('name', 'memberuserid2');
			$('#membername2').attr('name', 'membername2');
			$('#membersurname2').attr('name', 'membersurname2');
			$('#inputtext1member2').attr('name', 'text1member2');
			$('#inputtext2member2').attr('name', 'text2member2');
			$('#membertitle2').attr('name', 'text2member2');
			$('#membermail2').attr('name', 'text2member2');
			$('#memberisactive2').attr('name', 'memberisactive2');
			$('#memberisadmin-no2').attr('name', 'memberisadmin2');
		}
	});
}

function load_kids_functions(){
	$('#mdr-type').on('change', function(){
		var type=$(this).val();
		$('#inputtext2member'+admin_pos).val(type);
	});
...
function add_first_item_to_list(item_to_add, datos, type){
	console.log('add_first_item_to_list', datos);
	$.ajax({
...
(function ($) {
  window.ParsleyConfig = $.extend( true, {}, window.ParsleyConfig, {
    messages: {
      ...
      , minlength:      "Este valor es muy pequeño. Debe tener %s caracteres o más."
      , maxlength:      "Este valor es muy grande. Debe tener %s caracteres o menos."
      , rangelength:    "El tamaño de este valor es inválido. Debe tener entre %s y %s caracteres."
      , mincheck:       "Debe seleccionar por lo menos %s opciones."
      , maxcheck:       "Debe seleccionar %s opciones o menos."
      , rangecheck:     "Debe seleccionar entre %s y %s opciones."
      , equalto:        "Este valor debe ser el mismo."
      // parsley.extend ///////////////////////////////
      , minwords:       "Este valor debe tener como mínimo %s palabras."
      , maxwords:       "Este valor debe tener como máximo %s palabras."
...
/*
giftlistredirectpath=%2Fgiftlist%2Fmanage
giftlistid=
giftlistfileid=
giftlistfilepath=
giftlistisactive=True
giftlistaddress=
giftlisttype=4
giftlistname=Lista+demo+1
giftlisturl=listademo1
giftlistispublic=True
giftlistmessage=Ut+quam+metus%2C+congue+non+velit+vitae%2C+pulvinar+feugiat+orci.+Nam+non+lobortis+ante%2C+interdum+porttitor+nunc.+Vestibulum+ac+nisl+et+leo+feugiat+cursus+vel+quis+massa.+Duis+dignissim%2C+est+non+placerat+ullamcorper%2C+est+mauris+feugiat+purus%2C+maximus+porttitor+tellus+dolor+eu+quam.%0D%0A%0D%0AMorbi+eget+ultrices+tortor%2C+non+pulvinar+neque.+Aliquam+risus+arcu%2C+varius+id+interdum+eu%2C+pretium+ultrices+arcu.%0D%0A%0D%0AMorbi+quis+quam+ut+tortor+egestas+malesuada.+Morbi+quis+urna+scelerisque%2C+tincidunt+orci+vel%2C+suscipit+nisi.
giftlisteventdate=
giftlisteventlocation=
giftlisteventcity=
membernumber1=1
memberid1=
memberclientid1=
memberuserid1=c26f2738-491a-4f68-b464-b4f8411c2308
membertitle1=Wishlist
membername1=Ramiros
membersurname1=Garc%C3%ADa
membermail1=ramiro_garcia_rodriguez%40yahoo.com
text1member1=
text2member1=
memberisadmin1=True
memberisactive1=True
membernumberxmcx=xmcx
memberidxmcx=
memberclientidxmcx=
memberuseridxmcx=
membernamexmcx=
...
text1memberxmcx=
text2memberxmcx=
memberisadminxmcx=False
memberisactivexmcx=False
selected-deliveryAddress=C139970532944F6AA93A9C3B27B2A24E
giftlistaccept=on
giftlistv2cancelmessage=Deseja+realmente+abandonar+esta+p%C3%A1gina%3F+Todas+as+altera%C3%A7%C3%B5es+n%C3%A3o+salvas+ser%C3%A3o+perdidas.
*/